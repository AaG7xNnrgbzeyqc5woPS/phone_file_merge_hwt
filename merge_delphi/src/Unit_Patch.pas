...
function CalcCRC32(arr: TByteDynArray): uint32; overload;
...
function CalcCRC32(opfile: TFileStream; patch: type_xml;  var Readbuf: TByteDynArray): uint32; overload;
function test_CalcCRC32(filename: string; startbyte, len: int64): uint32;

procedure PatchToBuffer(patch: type_xml; var buffer: TByteDynArray);
procedure read_patch_data(opfile: TFileStream; patch: type_xml; var Readbuf: TByteDynArray);
...
begin
   ...
   s := Format('Count %d(0x%0:x) bytes CRC32, Values %d(0x%1:x)',[Len, Result]);
   MLog.device_log(s);

   s := Format('Print ALL CRC32 data (Length = %d)',[Len]);
   MLog.device_log(s);
   //MLog.PrintBytesHex(arr);
...
begin
  ...
  stream.Seek(startbyte, soBeginning);
  Result := CalcCRC32(stream, len);
...
begin
   ...
   s := Format('Count %d(0x%x) bytes CRC32, Values %d(0x%x)',[Len, Result]);
   MLog.device_log(s);
...
begin
  ...
  MLog.PrintPatching('开始解析xml文件');

  //解析补丁xml文件
  XMLFile    := TXMLFile.Create(patch_xml_filename, disk_size);
  XMLFile.XMLtype := enum_patch;     //当前是补丁操作，（编程操作：  enum_write enum_patch）
  XMLFile.Active  := True;

  MLog.PrintPatching('开始对每一个补丁文件进行处理！');
  //对每一个补丁文件做......
  Len := Length(XMLFile.patch_value);
  for i:=0 to Len-1 do
  begin
    patchfile := XMLFile.patch_value[i].filename;
    if Length(patchfile) <=0 then
    begin
      MLog.PrintPatchLog(i,patchfile,'文件名为空！跳过！');
      Continue;        //补丁文件名为空跳过
    end;
    if not FileExists(patchfile) then
    begin
      MLog.PrintPatchLog(i,patchfile,'文件不存在！跳过！');
      Continue;   //文件不存在也跳过
    end;

    MLog.PrintSpaceLine;
    MLog.device_log('----------------------------------------------------------');
    MLog.PrintPatchLog(i, patchfile, '补丁文件存在');
    patch_what := XMLFile.patch_value[i].what;
    MLog.device_log(' PATCH: (' + patchfile +') '+ patch_what);
    
    opFile := TFileStream.Create(patchfile, fmOpenReadWrite or fmShareExclusive);
    try
      MLog.device_log('Opened ' + patchfile + ' sizeof ' + IntToStr(opFile.Size) +' bytes');
      //with XMLFile.patch_value[i] do
      if XMLFile.patch_value[i].xml_function = 'CRC32' then
      begin
        //CRC32需要计算，补丁值是CRC32
        with XMLFile.patch_value[i] do
          s := Format('%s(%d,%d) requested', [xml_function,arg0,arg1]);
          
        MLog.device_log(s);

        crc32 := CalcCRC32(opFile, XMLFile.patch_value[i], Readbuf);
        XMLFile.patch_value[i].value := crc32;
        
        MLog.device_log('CRC32 Value （Memory Dump）:');
        MLog.PrintUintHex(crc32);
      end
      else
      begin
        //其它的就从补丁文件中，读补丁数据
        //XMLFile.patch_value[i].label_str 
        read_patch_data(opFile, XMLFile.patch_value[i], Readbuf);
      end;
...
function CalcCRC32(opfile: TFileStream; patch: type_xml; var Readbuf: TByteDynArray): uint32;
...
begin
  ...
  if PNumSectors = 0 then PNumSectors := 1;       //扇区数目为0，最少要取一个
  
  s := Format('StartSector: %d, length_inbytes: %d, NumSectors %d, ',
              [PStartSector, Patch.arg1, PNumSectors]);
  MLog.device_log('s');
  ...
  begin
    ...
    MLog.device_log('Complete Seek to ' + IntToStr(tmp) + ' bytes!' );
  end
  else
  begin
    opfile.seek(PStartSector*SECTOR_SIZE, soBeginning);
    MLog.device_log('Complete Seek to ' + IntToStr(PStartSector*SECTOR_SIZE) + ' bytes!' );
  end;

  //读出指定长度的数据保存到 Readbuf
  if Patch.arg0 > 64 then
  begin
    SetLength(Readbuf,64*SECTOR_SIZE);
    opfile.read(Readbuf[0],64*SECTOR_SIZE);
    MLog.device_log('Complete Read data to Readbuf, data Length by bytes  = ' + IntToStr(64*SECTOR_SIZE));
  end
  else
  begin
    SetLength(Readbuf, Patch.arg1);
    opfile.read(Readbuf[0],Patch.arg1);
    MLog.device_log('Complete Read data to Readbuf, data Length by bytes = ' + IntToStr(Patch.arg1));
  end;
...
procedure SeekBySector(opfile: TFileStream; start_sector: Int64);
begin
  if start_sector > 64  then
    start_sector := start_sector - 63;

   // TSeekOrigin = (soBeginning, soCurrent, soEnd);
  opfile.seek(start_sector * SECTOR_SIZE, soBeginning);
end;

//从补丁文件中将需要的补丁数据读到 Readbuf 缓冲区来
// pathfile =>  Readbuf
procedure Read_patch_data(opfile: TFileStream; patch: type_xml; var Readbuf: TByteDynArray);
var r_size,read_length : Int64;
    s : string;
    startPostionInByte : Int64;
begin
  //seek to start_sector
  startPostionInByte := patch.start_sector *  patch.SECTOR_SIZE_IN_BYTES;
  s := 'Move to ' + IntToStr(patch.start_sector) + ' Sectors'
      + ' (byte location ' + IntToStr(startPostionInByte)  + ') ';
  MLog.device_log(s);
  SeekBySector(opfile, patch.start_sector);
  ...
  s := 'success was able to read len(bytes_read)=' + IntToStr(read_length) + ' bytes';
  MLog.device_log(s);

  // 对于动态数组@Readbuf[0]才是数据真正开始的地方，@Readbuf[0] 和 @Readbuf的值是不同
  r_size := opfile.read(Readbuf[0], read_length);
  MLog.device_log('Reality Read len(bytes_read)= ' + IntToStr(r_size) + ' bytes');

  if  r_size <> read_length then
  begin
    MLog.device_log('文件尺寸小于需要读的长度！');
...
begin
  ...
  SeekBySector(opfile, patch.start_sector);
  MLog.device_log('Seek to start_sector: ' + IntToStr(patch.start_sector));

  //At sector 10 (0x1400) file_sector_offset 296 (0x128) in gpt_main0.bin with 7634910 (0x747FDE)
  s := Format('At sector %d (0x%.8x) file_sector_offset %d (0x%.8x) in %s with %d (0x%.8x)',
              [patch.start_sector,
               patch.start_sector *patch.SECTOR_SIZE_IN_BYTES,
               patch.byte_offset,
               patch.byte_offset,
               patch.filename,
               patch.value,
  ...
  s := Format('Patch Value: %d (0x%x)', [patch.value, patch.value]);
  MLog.device_log(s);
  
  //2, patch.value 的值 打补丁到  bytes_read 中对应处
  PatchToBuffer(Patch, buf);

  //MLog.device_log('打补丁后的缓冲区值'); //数据太多，关闭此输出
  // MLog.PrintBytesHex(buf);

  //3，将 bytes_read 写到文件中去
  MLog.device_log('(little endian)');
  MLog.device_log(Format('committing patch of length %d bytes',[Length(buf)]));
